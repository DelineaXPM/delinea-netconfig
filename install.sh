...
# Usage: curl -sfL https://raw.githubusercontent.com/DelineaXPM/delinea-platform/main/delinea-netconfig/install.sh | sh
...
LATEST_VERSION=$(curl -s https://api.github.com/repos/DelineaXPM/delinea-platform/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
DOWNLOAD_URL="https://github.com/DelineaXPM/delinea-platform/releases/download/${LATEST_VERSION}/${BINARY_NAME}"
